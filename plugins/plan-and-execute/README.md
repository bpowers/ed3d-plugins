...
A workflow plugin for Claude Code or Codex that guides you from rough idea to working implementation through structured design, planning, and execution phases.

## The Problem

Claude Code and Codex are excellent at implementing specific, well-defined tasks. But when you have a feature idea that's still forming - where you don't yet know exactly what you want, let alone how to build it - you need more structure. You need to explore alternatives, ground your design in the actual codebase, and break work into verifiable steps.
...
1. **Context Gathering** - The agent asks what you're building, what constraints exist, what you've already decided.
...
3. **Brainstorming** - The agent proposes 2-3 architectural approaches with trade-offs. A codebase investigator subagent finds existing patterns your design should follow. You pick an approach through incremental validation - small sections presented for feedback.
...
1. **Branch Setup** - The agent asks if you want a git worktree (isolated workspace) or standard branch. Creates the branch from main/master.
...
**Fresh context between phases.** In Claude Code, you /clear between design → plan and plan → execute. In Codex or other agents, start a fresh context/session between phases. Each phase gets full context for its specific job.
...
| Agent | Plugin | Role |
|-------|--------|------|
| **codebase-investigator** | plan-and-execute | Verifies file paths, finds patterns, confirms assumptions |
| **internet-researcher** | plan-and-execute | Finds current API docs, library patterns, best practices |
| **task-implementor** | plan-and-execute | Implements tasks with TDD, runs verification, commits |
| **code-reviewer** | plan-and-execute | Enforces quality standards, blocks on issues |
| **task-bug-fixer** | plan-and-execute | Fixes issues identified by code reviewer |
| **test-analyst** | plan-and-execute | Validates test coverage against acceptance criteria, generates human test plans |
| **project-claude-librarian** | plan-and-execute | Updates CLAUDE.md files when contracts change |
...
The agent will guide you through context gathering, brainstorming, and design documentation.

When design is complete, you'll get instructions to copy the next command, then clear/fresh-context your session:

```bash
# Copy this command first, then clear/fresh-context the session, then paste it
