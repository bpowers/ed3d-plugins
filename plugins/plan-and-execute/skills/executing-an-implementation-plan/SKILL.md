...
```
<invoke name="Task">
<parameter name="subagent_type">plan-and-execute:task-implementor</parameter>
...
<parameter name="prompt">
  ...
  2. Apply all relevant skills, such as (if available) plan-and-execute:coding-effectively
...
After all phases complete, invoke the `plan-and-execute:project-claude-librarian` subagent (when available) to review changes and update CLAUDE.md files if needed.

```
<invoke name="Task">
<parameter name="subagent_type">plan-and-execute:project-claude-librarian</parameter>
...
<parameter name="prompt">
  ...
  Follow the plan-and-execute:maintaining-project-context skill to:
...
**If librarian subagent is unavailable:** skip this entire step. Say aloud that you're skipping it because the `plan-and-execute` plugin is not available.
...
```
<invoke name="Task">
<parameter name="subagent_type">plan-and-execute:test-analyst</parameter>
